$(document).ready(function(){
  ...
  $image_crop_image5 = $('#image_demo_image5').croppie({
    enableExif: true,
    viewport: {
      width:250,
      height:250,
  ...
  $('#upload_product_image5').on('change', function() {
    var reader = new FileReader();
    reader.onload = function (event) {
      $image_crop_image5.croppie('bind', {
    ...
    if (!validImageTypes.includes(fileType)) {
      ...
      if (fileSize > 3) {
        ...
        $('#uploadimageModal_image5').modal('show');
      }
    }
  });

  $('.crop_image_image5').click(function(event){
    ...
    $image_crop_image5.croppie('result', {
      type: 'canvas',
      size: 'viewport'
      ...
      $.ajax({
        ...
        {
          $('#uploadimageModal_image5').modal('hide');
