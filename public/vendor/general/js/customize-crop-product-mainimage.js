$(document).ready(function(){
  ...
  $image_crop_mainimage = $('#image_demo_mainimage').croppie({
    enableExif: true,
    viewport: {
      width:250,
      height:250,
  ...
  $('#upload_product_main_image').on('change', function() {
    var reader = new FileReader();
    reader.onload = function (event) {
      $image_crop_mainimage.croppie('bind', {
    ...
    if (!validImageTypes.includes(fileType)) {
      ...
      if (fileSize > 3) {
        ...
        $('#uploadimageModal_mainimage').modal('show');
      }
    }
  });

  $('.crop_image_mainimage').click(function(event){
    ...
    $image_crop_mainimage.croppie('result', {
      type: 'canvas',
      size: 'viewport'
      ...
      $.ajax({
        ...
        {
          $('#uploadimageModal_mainimage').modal('hide');
          $('#uploaded_mainimage').html(data);
        }
      });
    })
  });
