<x-app-layout>
    ...
    @section('script')
        ...
        <script>
            ...
            function getDetails() {
                var url="{{ url('/dashboard/finance/purchase/order/details/') }}"+"/"+{{$id}};
                ...
                $.ajax({
                    ...
                    success: function(res){
                        ...
                        $('#title').html(res.purchase_order.kind+" #"+res.purchase_order.transaction_no);
                        $('#status').html(res.purchase_order.transaction_status.name);
                        $('#receiver').html("customer");
                        $('#receiver_name').html(res.purchase_order.person.display_name);
                        $('#address').html(res.purchase_order.shipping_address);
                        $('#email').html(res.purchase_order.email);
                        $('#transaction_date').html(res.purchase_order.transaction_date);
                        $('#due_date').html(res.purchase_order.due_date);
                        $('#term').html(res.purchase_order.term.name);
                        if(res.purchase_order.purchase_order){
                            $('#order_no').html(res.purchase_order.purchase_order.transaction_no);

                        }
                        $('#transaction_no').html(res.purchase_order.transaction_no);
                        $('#ref_no').html(res.purchase_order.reference_no);
                        $('#tags').html(res.purchase_order.tags);
                        $.each(res.purchase_order.transaction_lines_attributes,function (key, value) {
                        ...
                        $('#subtotal').html(res.purchase_order.subtotal_currency_format);
                        $('#Total_amount').html(res.purchase_order.original_amount_currency_format);
                        $('#payment_paid').html(res.purchase_order.payment_received_amount_currency_format);
                        $('#balance_due').html(res.purchase_order.credit_memo_balance_currency_format);
                    },
                    error: function(data){
                        console.log(data);
                    }
