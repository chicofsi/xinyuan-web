<x-app-layout>
            ...
            <div class="panel-body invoice">
                <div class="row">
                    <div class="col-md-8 col-sm-8">
                        <h1 class="invoice-title" id="title">order</h1>
                    </div>
    ...
    @section('script')
        ...
        <script>
            ...
            function getDetails() {
                var url="{{ url('/dashboard/finance/sales/order/details/') }}"+"/"+{{$id}};
                ...
                $.ajax({
                    ...
                    success: function(res){
                        ...
                        $('#title').html(res.sales_order.kind+" #"+res.sales_order.transaction_no);
                        $('#status').html(res.sales_order.transaction_status.name);
                        $('#receiver').html("customer");
                        $('#receiver_name').html(res.sales_order.person.display_name);
                        $('#address').html(res.sales_order.shipping_address);
                        $('#email').html(res.sales_order.email);
                        $('#transaction_date').html(res.sales_order.transaction_date);
                        $('#due_date').html(res.sales_order.due_date);
                        $('#term').html(res.sales_order.term.name);
                        if(res.sales_order.sales_order){
                            $('#order_no').html(res.sales_order.sales_order.transaction_no);

                        }
                        $('#transaction_no').html(res.sales_order.transaction_no);
                        $('#ref_no').html(res.sales_order.reference_no);
                        $('#tags').html(res.sales_order.tags);
                        $.each(res.sales_order.transaction_lines_attributes,function (key, value) {
                        ...
                        $('#subtotal').html(res.sales_order.subtotal_currency_format);
                        $('#Total_amount').html(res.sales_order.original_amount_currency_format);
                        $('#payment_paid').html(res.sales_order.payment_received_amount_currency_format);
                        $('#balance_due').html(res.sales_order.credit_memo_balance_currency_format);
                    },
                    error: function(data){
                        console.log(data);
                    }
