<x-app-layout>
            ...
            <div class="panel-body invoice">
                <div class="row">
                    <div class="col-md-8 col-sm-8">
                        <h1 class="quote-title" id="title">Sales Quote</h1>
                    </div>
    ...
    @section('script')
        ...
        <script>
            ...
            function getDetails() {
                var url="{{ url('/dashboard/finance/sales/quote/details/') }}"+"/"+{{$id}};
                ...
                $.ajax({
                    ...
                    success: function(res){
                        ...
                        $('#title').html(res.sales_quote.kind+" #"+res.sales_quote.transaction_no);
                        $('#status').html(res.sales_quote.transaction_status.name);
                        $('#receiver').html("customer");
                        $('#receiver_name').html(res.sales_quote.person.display_name);
                        $('#address').html(res.sales_quote.shipping_address);
                        $('#email').html(res.sales_quote.email);
                        $('#transaction_date').html(res.sales_quote.transaction_date);
                        $('#due_date').html(res.sales_quote.due_date);
                        $('#term').html(res.sales_quote.term.name);
                        if(res.sales_quote.sales_order){
                            $('#order_no').html(res.sales_quote.sales_order.transaction_no);

                        }
                        $('#transaction_no').html(res.sales_quote.transaction_no);
                        $('#ref_no').html(res.sales_quote.reference_no);
                        $('#tags').html(res.sales_quote.tags);
                        $.each(res.sales_quote.transaction_lines_attributes,function (key, value) {
                        ...
                        $('#subtotal').html(res.sales_quote.subtotal_currency_format);
                        $('#Total_amount').html(res.sales_quote.original_amount_currency_format);
                        $('#payment_paid').html(res.sales_quote.payment_received_amount_currency_format);
                        $('#balance_due').html(res.sales_quote.credit_memo_balance_currency_format);
                    },
                    error: function(data){
                        console.log(data);
                    }
