<x-app-layout>
    ...
    <div class="wrapper">
            ...
            <div class="panel-body invoice">
                <div class="row">
                    <div class="col-md-8 col-sm-8">
                        <h1 class="invoice-title" id="title">invoice</h1>
                    </div>
                ...
                <div class="invoice-address">
                    <div class="row">
                        ...
                        <div class="col-md-4  col-sm-4">
                            <div class="inv-col"><span>Sales Order No </span><div id="order_no">Sales Order #10001</div> </div>
        ...
        <div class="text-right ">
            <a class="btn btn-info btn-lg" id="editButton"><i class="fa fa-pencil"></i> Edit Invoice </a>
        </div>
    ...
    @section('script')
        ...
        <script>
            ...
            function getDetails() {
                var url="{{ url('/dashboard/finance/sales/invoice/details/') }}"+"/"+{{$id}};
                ...
                $.ajax({
                    ...
                    success: function(res){
                        ...
                        $('#title').html(res.sales_invoice.kind+" #"+res.sales_invoice.transaction_no);
                        $('#status').html(res.sales_invoice.transaction_status.name);
                        $('#receiver').html("customer");
                        $('#receiver_name').html(res.sales_invoice.person.display_name);
                        $('#address').html(res.sales_invoice.shipping_address);
                        $('#email').html(res.sales_invoice.email);
                        $('#transaction_date').html(res.sales_invoice.transaction_date);
                        $('#due_date').html(res.sales_invoice.due_date);
                        $('#term').html(res.sales_invoice.term.name);
                        if(res.sales_invoice.sales_order){
                            $('#order_no').html(res.sales_invoice.sales_order.transaction_no);

                        }
                        $('#transaction_no').html(res.sales_invoice.transaction_no);
                        $('#ref_no').html(res.sales_invoice.reference_no);
                        $('#tags').html(res.sales_invoice.tags);
                        $.each(res.sales_invoice.transaction_lines_attributes,function (key, value) {
                        ...
                        $('#subtotal').html(res.sales_invoice.subtotal_currency_format);
                        $('#Total_amount').html(res.sales_invoice.original_amount_currency_format);
                        $('#payment_paid').html(res.sales_invoice.payment_received_amount_currency_format);
                        $('#balance_due').html(res.sales_invoice.credit_memo_balance_currency_format);
                        $('#editButton').attr('href',"{{ url('dashboard/finance/sales/invoice/edit').'/'.$id }}")
                    },
                    error: function(data){
                        console.log(data);
                    }
