            ...
            <div class="panel">
                <div class="panel-body invoice">
                    <div class="row">
                        <div class="col-md-8 col-sm-8">
                            <h2 class="invoice-title" id="title">Update {{$invoice->sales_invoice->kind." #".$invoice->sales_invoice->transaction_no}}</h2>
                        </div>
                        <div class="col-md-4 col-sm-4" style="padding-top: 15px">
                            <h1 class="t-due">Total Amount </h1>
                            <h2 class="amnt-value" id="total_amount">{{intval($invoice->sales_invoice->original_amount)}}</h2>
                        </div>
                    </div>
                    <div class="invoice-address">
                        <div class="row">
                            <div class="col-md-4 col-sm-4">
                                ...
                                <h2 class="corporate-id" >
                                    <select class="selectpicker  form-control" data-live-search="true" id="person_name" style="color: black;" name="person_name" >
                                        @foreach ($customers->customers  as $val)

                                            <option value="{{$val->display_name}}" @if ($invoice->sales_invoice->person->id == $val->id)
                                                selected
                                            @endif>{{$val->display_name}}</option>
                                        @endforeach
                                    </select>
                                </h2>
                                <h4 class="inv-to">Email</h4>

                                <input type="email" class="form-control" id="email" value="{{$invoice->sales_invoice->email}}" placeholder="Enter email">
                                <h4 class="inv-to" >Billing Address</h4>
                                <textarea rows="3" id="address" class="form-control">{{$invoice->sales_invoice->address}}</textarea>
                            </div>
                            <div class="col-md-4 col-sm-4">
                                <h4 class="inv-to" id="receiver">Transaction Date</h4>
                                <input data-date-format="mm/dd/yyyy" class="form-control default-date-picker" style="color: black;" name="transaction_date" id="transaction_date" type="text" value="{{DateTime::createFromFormat('d/m/Y', $invoice->sales_invoice->transaction_date)->format('m/d/Y')}}" />
                                <h4 class="inv-to" >Due Date</h4>
                                <input data-date-format="mm/dd/yyyy" class="form-control default-date-picker" style="color: black;" name="due_date" id="due_date" type="text" value="{{DateTime::createFromFormat('d/m/Y', $invoice->sales_invoice->due_date)->format('m/d/Y')}}" />
                                <h4 class="inv-to">Term</h4>
                                <select class="desk form-control" id="terms" style="color: black;" name="terms" >
                                    @foreach ($terms->terms as $val)
                                        <option value="{{$val->id}}" @if ($invoice->sales_invoice->term->id == $val->id)
                                                selected
                            ...
                            <div class="col-md-4 col-sm-4">
                                <h4 class="inv-to" id="receiver">Customer Ref No </h4>
                                <input type="text" class="form-control" id="reference_no" placeholder="Enter Customer Reference No" value="{{$invoice->sales_invoice->reference_no}}">
                                <h4 class="inv-to" >Warehouse</h4>
                                <select class="desk form-control" id="warehouse" style="color: black;" name="warehouse" >
                                    <option value="" @if ($invoice->sales_invoice->warehouse->id == null)
                                                selected
                                            @endif>No Warehouse</option>
                                    @foreach ($warehouses->warehouses as $val)
                                        <option value="{{$val->id}}" @if ($invoice->sales_invoice->warehouse->id == $val->id)
                                                selected
                                            @endif>{{$val->name}}</option>
                                    @endforeach
                                </select>
                ...
                <table class="table table-bordered table-invoice">
                    ...
                    <tbody id="product_table">
                        @foreach ($invoice->sales_invoice->transaction_lines_attributes as $val)
                        
                        <tr>
                            <td>
                                <select class="desk form-control productlist" id="product" style="color: black;" name="product" >
                                    @foreach ($products->products as $v)
                                        <option value='{{$v->id}}' @if ($val->product->id == $v->id)
                                                selected
                                            @endif>{{$v->name}}</option>
                                    @endforeach
                                </select>
                                <textarea rows="1" id="description" class="form-control">{{$val->description}}</textarea>
                            </td>
                            <td>
                                <input type="number" class="form-control" id="qty" placeholder="Enter Qty" value="{{intval($val->quantity)}}" min="0">
                            </td>
                            <td>
                                <select class="desk form-control" id="units" style="color: black;"  name="units">
                                </select>
                            </td>
                            <td>
                                <input type="number" class="form-control" id="unit_price" value="{{intval($val->rate)}}" min="0">
                            </td>
                            <td>
                                <input type="number" class="form-control" id="discount" value="{{intval($val->discount)}}" min="0">
                            </td>
                            <td>
                                <input type="number" class="form-control" id="total" value="{{intval($val->amount)}}" min="0">
                            </td>
                            <td>
                                <a href="javascript:void(0)" data-toggle="tooltip" data-original-title="detail" class="btn btn-danger btn-delete btn-sm"><i class="fas fa-trash"></i></a>
                            </td>
                        </tr>
                    ...
                    <tfoot>
                        ...
                        <tr>
                            <td colspan="2" style="text-align: left !important">
                                ...
                                <textarea rows="3" id="memo" class="form-control">{{$invoice->sales_invoice->memo}}</textarea>
                                <h4 class="inv-to">Message</h4>
                                <textarea rows="3" id="message" class="form-control">{{$invoice->sales_invoice->message}}</textarea>
                            ...
                            <td class="text-center">
                                <p id="subtotal">{{intval($invoice->sales_invoice->subtotal)}}</p>
                                <p id="discount_all">{{intval($invoice->sales_invoice->discount_price)}}</p>
                                <p><strong id="total_all">{{intval($invoice->sales_invoice->original_amount)}}</strong></p>
                            </td>
                        </tr>
                    </tfoot>
                </table>
    ...
    @section('script')
        ...
        <script>
            ...
            $('#formInvoice').submit(function(e) {
                ...
                $.ajax({
                    type:'POST',
                    url: "{{ url('/dashboard/finance/sales/invoice/edit')."/".$invoice->sales_invoice->id}}",
                    data: form,
                    cache:false,
                    contentType: false,
                    processData: false,
