...
class ManageProductValue extends Controller
{
    ...
    {
        $purchasedetails=PurchaseDetails::with('product','purchase')->get();
        $product=Product::with('warehouseproduct','purchasedetails')->get();

        return view('admin.product.value',compact('product','purchasedetails'));
    }
    ...
    public function list(Request $request)
    {
        ...
        if($purchase->isEmpty()){
            ...
            foreach ($purchase as $key => $value) {
                foreach ($value->purchasedetails as $k => $v) {
                    $product=Product::where('id',$v->id_product)->with('warehouseproduct')->first();
                    $product->name=ProductType::where('id',$product->id_type)->first()->name." ".ProductSize::where('id',$product->id_size)->first()->width."X".ProductSize::where('id',$product->id_size)->first()->height." ".Factories::where('id',$product->id_factories)->first()->name." ".ProductColour::where('id',$product->id_colour)->first()->name." ".ProductLogo::where('id',$product->id_logo)->first()->name;
                    $data.= "<tr>
                            <td>".$value->date."</td>
                            <td>".$product->name."</td>
                            <td>".$value->currency->symbol.number_format(intval($v->price), 0, ',', ',')."</td>
                            <td>Rp. ".number_format(intval($v->price_idr), 0, ',', ',')."</td>
                            <td>".$v->quantity."</td>
                            <td>Rp. ".number_format(intval($v->price_idr*$v->quantity), 0, ',', ',')."</td>
                        </tr>";
                }
            }
    ...
    public function store(Request $request)
    {
        ...
        if($request->payment == "cash"){
            $purchasepayment = PurchasePayment::create([
                ...
                'date' => date("Y-m-d",strtotime($request->date)),
                'paid' => $request->total_payment_idr
            ]);
    ...
    public function edit(Request $request)
    {
        ...
        foreach ($request->details as $key => $value) {
            ...
            $price_idr=$value->price*$value->rates;
    ...
    public function storeProduct(Request $request)
    {
        ...
        $price_idr=$request->price*$request->rates;
